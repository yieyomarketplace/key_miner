            ...
            for match in pattern.finditer(content):
                ...
                if confidence >= self.min_confidence:
                    finding = {
                        ...
                        "raw_match": matched_text,  # for dedup, if needed
                    }
...
class GitHubMiner:
                                ...
                                for f in file_findings:
                                    ...
                                    if self.detector.dedup:
                                        if secret_hash not in seen_secrets:
                                            ...
                                            # Remove raw_match before storing? We'll keep it but not in output.
                                            # We'll store without raw_match in final output.
                                            findings.append(f)
                                    else:
                                        findings.append(f)
    ...
    def _parse_retry_after(self, exception: GithubException) -> Optional[int]:
        """Extract Retry-After header from exception if present."""
...
def main():
    ...
    def signal_handler(sig, frame):
        ...
        store.finalize()
        # Also save state if resume enabled
    ...
    total_findings = 0
    for repo in repos:
        # Skip if already processed (state check already done in search, but double-check)
        ...
        # Mark repo as processed (if resume)
        if args.resume and miner.state_mgr:
            miner.state_mgr.mark_repo_done(repo.full_name, len(findings), files_scanned)

        # If flush interval is set to 0, we rely on flush_on_repo; but if both false, we still flush at end.
        # We already flush in add_findings if conditions met.

    # Final flush
    store.finalize()

    # Save state if resume
    if args.resume and miner.state_mgr:
        miner.state_mgr.save()
